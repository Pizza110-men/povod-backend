...
# Neon/Render дают строку вида postgres://... — приводим к формату SQLAlchemy
...
class User(Base):
    ...
    avatar: Mapped[str] = mapped_column(String(300), default="")  # ссылка на фото TG или эмодзи
...
def telegram_login(data: TelegramAuth, db: Session = Depends(get_db)):
    ...
    if user is None:
        user = User(telegram_id=tg_user["id"],
                    ...
                    avatar=(tg_user.get("photo_url") or "")[:300])
        db.add(user)
        db.commit()
        db.refresh(user)
    elif not user.avatar and tg_user.get("photo_url"):
        user.avatar = tg_user["photo_url"][:300]
        db.commit()
    if user.is_banned:
...
@app.get("/povods/mine", response_model=list[PovodPublic], tags=["povods"])
def my_povods(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """Все поводы текущего пользователя — профиль больше ничего не забывает."""
    q = (select(Povod).options(joinedload(Povod.author))
         .where(Povod.author_id == user.id)
         .order_by(Povod.created_at.desc()).limit(100))
    return db.scalars(q).all()
